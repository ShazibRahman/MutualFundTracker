...
class InvestmentHistoryRepository:
    ...
    @staticmethod
    def find_by_mfid_and_date(
        mfid: str, entry_date: date
    ) -> Optional[InvestmentHistory]:
        with Session(engine) as session:
            statement = select(InvestmentHistory).where(
                InvestmentHistory.mfid == mfid, InvestmentHistory.date == entry_date
            )
    ...
    def find_all_by_mfid(
        mfid: str, order_by: str = "date", direction: OrderType = "asc"
    ...
    def find_all_distinct_mfids_and_name() -> List[dict[str, str]]:
        with Session(engine) as session:
            statement = select(
                InvestmentHistory.mfid, InvestmentHistory.mfname
            ).distinct()
    ...
    def find_latest_records_for_all_mfids() -> Sequence[InvestmentHistory]:
        ...
        with Session(engine) as session:
            ...
            statement = select(InvestmentHistory).join(
                subquery,
    ...
    @staticmethod
    def find_second_latest_by_mfid(mfid: str) -> Optional[InvestmentHistory]:
        with Session(engine) as session:
