...
def percentage(string: str) -> html.Font:
    ...
    return html.Font(
        f"({string})%",
        style={"font-weight": "bold", "color": "green" if float(string) > 0 else "red"},
    )
...
def prepare_summary_table(summary_table) -> list[html.Table]:
    ...
    for x in range(len(summary_table)):
        if x == 0:
            children_sum_tab.append(
                html.Thead(html.Tr([html.Th(x) for x in summary_table[x]]))
            )
        else:
            tr = html.Tr()
            tr.children = []
            for y in range(len(summary_table[x])):
                if y == 2:
                    ...
                    td.children = [
                        html.Font(
                            summary_table[x][y],
                            style={"font-weight": "bold", "color": "#ac6f05"},
                        )
                    ]
                else:
...
def prepare_children_mut_tab(MutualFund_table):
    ...
    for x in range(len(MutualFund_table)):
        if x == 0:
            children_mut_tab.append(
                html.Thead(html.Tr([html.Th(x) for x in MutualFund_table[x]]))
            )
        else:
            # children_mut_tab.append(html.Tbody(
            #     html.Tr([html.Td(x) for x in MutualFund_table[x]])))
            tr = html.Tr(style={"color": "black"})
            tr.children = []
            ...
            for y in range(len(MutualFund_table[x])):
                if y == 1:
                    ...
                    td.children = [
                        ...
                        html.Br(),
                        html.Br(),
                        percentage(_2) if _2 != "N.A." else _2,
                    ]
                elif y == 3:
                    _1, _2 = MutualFund_table[x][y].split(" ")
                    td = html.Td()
                    td.children = [
                        ...
                        html.Br(),
                        html.Font(
                            "₹" + _2, style={"font-weight": "bold", "color": "black"}
                        ),
                    ]
                elif y == 4:
                    _1, _2 = MutualFund_table[x][y].split(" ")
                    ...
                    td.children = [
                        html.Font(
                            _1, style={"font-weight": "bold", "color": "#ac6f05"}
                        ),
                        html.Br(),
                        html.Br(),
                        _2,
                    ...
                    td = html.Td(MutualFund_table[x][y])
...
layout = dbc.Container(
    children=[
                        ...
                        dbc.Col(
                            children=[
                                dcc.Graph(
                                    id="my-graph4",
                                    figure=getPieChart(
                                        helper.getInvestmentDistribution()[0], "initial"
                                    ),
                                )
                            ],
                        ...
                        dbc.Col(
                            children=[
                                dcc.Graph(
                                    id="my-graph5",
                                    figure=getPieChart(
                                        helper.getInvestmentDistribution()[1], "current"
                                    ),
                                )
                            ],
                            className="col-12 col-md-6",
        ...
        html.Div(
            children=prepareTable(), className="container mt-5 mb-5  table-responsive"
        ),
    ],
    className="container",
)
