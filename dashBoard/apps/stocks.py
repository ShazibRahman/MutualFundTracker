...
layout = html.Div(
    [
                        ...
                        dbc.Col(
                            [
                                dcc.DatePickerRange(
                                    ...
                                    initial_visible_month=date(2021, 8, 23),
                ...
                dbc.Col(
                    [
                        dcc.Dropdown(
                            id="dropdown",
                            optionHeight=60,
                            options=helper.get_all_stocks_list(),
                            value="",
                            multi=False,
        ...
        html.Div(
            [stock_helper.get_stock_data_in_form_of_table()], className="container"
        ),
    ],
    className="container",
)
...
@app.callback(
    ...
    [Input("add", "n_clicks")],
    [State("dropdown", "value"), State("units", "value"), State("amount", "value")],
    prevent_initial_call=True,
)
def add_stock_order(n_clicks, dropdown, units, amount):
    ...
    if booolean := helper.add_order_stock(dropdown, int(units), float(amount)):
        return (
            html.Div(
                f"Order for {helper.get_all_stock_dic()[dropdown]} units {units} at amount per units {amount}",
                style={
                    "color": "green",
                    "font-weight": "bold",
                    "font-size": "20px",
        ...
        return html.Div(
            "Order not added because there is no such Stock",
            style={
                "color": "red",
                "font-weight": "bold",
                "font-size": "20px",
